...
doc_type_list = sys.argv[3]
print("Started at : ",datetime.now())

print("The parent directory under process is :", directory)
print("The Outputs will be recorded at parent location :", report_directory)
print("The document types available :", doc_type_list)

for index, item in enumerate(doc_type_list.split(',')):
    print(index,' : ',item)

doctype_dict  = {key:value for key,value in enumerate(doc_type_list.split(','))}
#print(doctype_dict)

val = input("\nEnter the Document Type you wish to process further: ")

doc_type=[value for key,value in doctype_dict.items() if str(key)==str(val)][0]
	
directory = os.path.join(directory,doc_type)
report_directory = os.path.join(report_directory,doc_type)

print("The directory under process is :", directory)
print("The Outputs will be recorded at location :", report_directory)
print("The document types being processed :", doc_type)
    ...
    for filename in files:
        ...
        if filename.endswith('.json'):
            ...
            for ele in pageInfoList_json:
                ...
                file_name_xml = ele['id'] +"-"+ page_no_str + ".xml"
                file_name_xml = file_name_xml.split('#')
                file_name_xml = doc_type+'-'+ file_name_xml[1]
                print('-----------------------------------------',file_name_xml)
                ...
                print('-----------------------------------------',new_file_name)
                new_file_name_xml = path_tuple[0]+"_"+path_tuple[1]+"_"+file_name_xml
                new_file_path = class_namewise_doc_directory_path+'/'+bucket_dir+'\\'+folder_name_dir+"\\"+dt_name_dir+'\\'+new_file_name
                #new_file_path.replace('\\','/')
                print('----------------from-------------------------',root+"/"+file_name)
                print('-----------------to------------------------',new_file_path)

                new_file_path_xml = os.path.join(class_namewise_doc_directory_path,bucket_dir,folder_name_dir,dt_name_dir)+"\\"+new_file_name_xml
                ...
                if os.path.exists(root+"\\"+file_name_xml):
                        shutil.copyfile(root+"\\"+file_name_xml,os.path.join(class_namewise_doc_directory_path,bucket_dir,folder_name_dir,dt_name_dir)+"\\"+new_file_name_xml)
        ...
        #-----Write to MongoDB ----------------------------------------------------------------------            
                ...
                if file_name.endswith('.pdf'):
                    ...
                    if use_mongo_db=='1':
                        mycol.update_one({"PAGEID":mongo_pageid},{"$set":{"preclassification_flag":"1","fileloc":root,"classname":class_name_dir,"bucket_name":bucket_dir,"classified_new_location":new_file_path}})
        #-----------------------------------------------------------------------------------------                
        count=count+1
# =================================================================================================================================
