...
directory = sys.argv[1]#'F:    est_Classification'
report_directory = sys.argv[2] #'F:    est_Classification_Output'
...
for root, dirs, files in os.walk(directory):
    ...
    for filename in files:
        ...
        dt_name_dir = doc
        print(root)
        xmlfilelist_cnt = [f for f in os.listdir(root) if f.endswith('.xml')]
        if len(xmlfilelist_cnt) > 0:
            if filename.endswith('.json'):
                ...
                for ele in pageInfoList_json:
                    ...
                    import csv
                    with open('taxreturns_classification_exemption_list.csv', mode='r') as infile_exemption:
                    ...
                    f = open("taxreturns_classname_correction_mappings.csv", "r")
                    with open('taxreturns_classname_correction_mappings.csv', mode='r') as infile:
                    ...
                    file_name = 'Credit Supplement-'+ file_name[1]
                    print(file_name)
                    
                    file_name_xml = ele['id'] +"-"+ page_no_str + ".xml"
                    file_name_xml = file_name_xml.split('#')
                    file_name_xml = 'Credit Supplement-'+ file_name_xml[1]
                    print(file_name_xml)
