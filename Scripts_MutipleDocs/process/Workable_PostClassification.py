...
def incremental_doc(filename):
    print("inside def ",filename)
    if use_mongo_db=='1':
        print("inside def mongo")
        if filename.endswith('.pdf'):
            print("inside def pdf if")
            ...
            print(mongo_pageid)
            abc = mycol.find({'PAGEID':mongo_pageid,"postclassification_flag": 1})
            match_cnt = len(list(abc))
            print('--->',match_cnt)
            if match_cnt > 0:
                incrementall_flag=0
                print('if-----',incrementall_flag)
            else:
                incrementall_flag=1
                print('else-----',incrementall_flag)
