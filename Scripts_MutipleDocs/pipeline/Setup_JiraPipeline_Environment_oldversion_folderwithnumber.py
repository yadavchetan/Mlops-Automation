...
if val=='2':
    ...
    proc_productiondownload_loc = os.path.join(ade_dwnld_jira_loc,'0_ProductionDownload')
    os.mkdir(proc_productiondownload_loc)
    proc_decryption_loc = os.path.join(ade_dwnld_jira_loc,'1_Decryption')
    os.mkdir(proc_decryption_loc)
    
    proc_Classification_loc = os.path.join(ade_dwnld_jira_loc,'2_Classification')
    os.mkdir(proc_Classification_loc)
    proc_Preprocessing_loc = os.path.join(ade_dwnld_jira_loc,'3_Preprocessing')
    ...
    for doctypenames in var_doctype_list:
        ...
        proc_Preprocessing_Conversion_loc = os.path.join(preprocessing_doctypewise_path,'1_Conversion')
        os.mkdir(proc_Preprocessing_Conversion_loc)
        proc_Preprocessing_deskew_loc = os.path.join(preprocessing_doctypewise_path,'2_Deskew')
        os.mkdir(proc_Preprocessing_deskew_loc)
        proc_Preprocessing_deduplicate_loc = os.path.join(preprocessing_doctypewise_path,'3_De-Duplication')
        os.mkdir(proc_Preprocessing_deduplicate_loc)
...
if val=='3':
    ...
    proc_preprocess_loc = os.path.join(ade_aiq_jira_loc,'1_PreProcessing')
    ...
    proc_prerenaming_loc = os.path.join(ade_aiq_jira_loc,'2_PreRenaming')
    os.mkdir(proc_prerenaming_loc)
    proc_docuploaded_loc = os.path.join(ade_aiq_jira_loc,'3_Documents_Uploaded')
    os.mkdir(proc_docuploaded_loc)
    proc_docdownloaded_loc = os.path.join(ade_aiq_jira_loc,'4_Documents_Downloaded')
    os.mkdir(proc_docdownloaded_loc)
    proc_invalidxml_loc = os.path.join(ade_aiq_jira_loc,'5_Invalidated_XML')
    proc_invalidxml_dump = os.path.join(proc_invalidxml_loc,'dump')
    os.mkdir(proc_invalidxml_loc)
    os.mkdir(proc_invalidxml_dump)
    proc_postclassification_loc = os.path.join(ade_aiq_jira_loc,'6_Classification')
    os.mkdir(proc_postclassification_loc)
    proc_postrenaming_loc = os.path.join(ade_aiq_jira_loc,'7_PostRenaming')
    os.mkdir(proc_postrenaming_loc)
    proc_postprocessing_loc = os.path.join(ade_aiq_jira_loc,'8_PostProcessing')
    os.mkdir(proc_postprocessing_loc)
    proc_transfera51_loc = os.path.join(ade_aiq_jira_loc,'9_TransferToArea51')
