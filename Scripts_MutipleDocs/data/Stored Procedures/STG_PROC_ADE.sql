USE DATABANKDB;
DROP PROCEDURE IF EXISTS STG_PROC_ADE;
...
CREATE PROCEDURE STG_PROC_ADE()
BEGIN
	
	DECLARE EXIT HANDLER FOR 1146
	  SELECT 'Please create table abc first' Message;
	TRUNCATE TABLE STG_TBL_ADE;
	SELECT 'Table truncated successfully' Message;

	INSERT INTO STG_TBL_ADE (PIPLELINE, CLIENT_NAME, REFERENCE_KEY, PROCESS, SUB_PROCESS, LOAN_ID, DOCUMENT_ID, PAGE_ID, JIRA_ID, START_DATE, END_DATE, PROCESS_DATE, DOCUMENT_TYPE, PAGE_NO, EXTENSION)
	SELECT 
		PIPLELINE, 
		CLIENT_NAME, 
		REFERENCE_KEY, 
	...
	from SRC_TBL_ADE;
	SELECT 'Record successfully Inserted In STG_TBL_ADE' Message;
